...
function BlogPost({ post }: { post: Post }) {
    ...
    return (
        <div style={{ background: '#0a0a0a', minHeight: '100vh', color: '#fff', fontFamily: "'Inter', sans-serif" }}>
            {/* Back nav */}
            <nav style={{ position: 'fixed', top: 0, left: 0, right: 0, zIndex: 999, background: 'rgba(8,8,8,0.95)', backdropFilter: 'blur(20px)', borderBottom: '1px solid rgba(230,0,0,0.12)', padding: '0 2rem', height: 64, display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                <a href="/" style={{ display: 'flex', alignItems: 'center', gap: 12, textDecoration: 'none' }}>
                    <img src={logo} alt="Norcoches" style={{ height: 40, objectFit: 'contain' }} />
                    <span style={{ color: '#e60000', fontWeight: 900, fontSize: '1.1rem' }}>Norcoches</span>
                </a>
            ...
            <div style={{ maxWidth: 780, margin: '0 auto', padding: '100px 1.5rem 4rem' }}>
                ...
                {post.tags?.length > 0 && (
                    <div style={{ display: 'flex', flexWrap: 'wrap', gap: 8, marginBottom: 20 }}>
                        {post.tags.map(tag => (
                            <span key={tag} style={{ background: 'rgba(230,0,0,0.12)', color: '#ff6b6b', fontSize: '0.75rem', fontWeight: 700, padding: '4px 12px', borderRadius: 20, border: '1px solid rgba(230,0,0,0.25)', textTransform: 'uppercase', letterSpacing: 1 }}>{tag}</span>
                        ))}
                    </div>
                )}
                ...
                <div style={{ display: 'flex', alignItems: 'center', gap: 16, marginBottom: 32, color: 'rgba(255,255,255,0.4)', fontSize: '0.85rem', flexWrap: 'wrap' }}>
                    <span>✍️ {post.author || 'Norcoches'}</span>
                    <span>•</span>
                ...
                {/* CTA */}
                <div style={{ marginTop: 56, padding: '2rem', borderRadius: 18, background: 'linear-gradient(135deg, rgba(230,0,0,0.1), rgba(179,0,0,0.05))', border: '1px solid rgba(230,0,0,0.2)', textAlign: 'center' }}>
                    <p style={{ marginBottom: 16, fontWeight: 700, fontSize: '1.1rem' }}>Prêt à louer votre voiture à Tanger ?</p>
                    <a href="/" style={{ display: 'inline-block', background: 'linear-gradient(45deg, #b30000, #e60000)', color: '#fff', padding: '0.75rem 2rem', borderRadius: 12, fontWeight: 900, textDecoration: 'none', fontSize: '1rem' }}>
                        Voir notre flotte →
                    </a>
                </div>
            </div>
        ...
        .blog-content h3 { color: #e60000; font-size: 1.2rem; font-weight: 800; margin: 2rem 0 0.75rem; }
        .blog-content p { margin-bottom: 1.25rem; }
        .blog-content ul, .blog-content ol { margin: 1rem 0 1rem 1.5rem; }
        .blog-content li { margin-bottom: 0.5rem; }
        .blog-content strong { color: #fff; font-weight: 800; }
        .blog-content a { color: #e60000; }
        .blog-content blockquote { border-left: 3px solid #e60000; padding-left: 1.25rem; margin: 1.5rem 0; color: rgba(255,255,255,0.55); font-style: italic; }
...
function BlogList({ posts }: { posts: Post[] }) {
    useEffect(() => {
        document.title = 'Blog Norcoches – Conseils & Guide Location Voiture à Tanger';
        window.scrollTo(0, 0);
    }, []);
    ...
    return (
        <div style={{ background: '#0a0a0a', minHeight: '100vh', color: '#fff', fontFamily: "'Inter', sans-serif" }}>
            ...
            {/* Hero */}
            <section style={{ padding: '120px 1.5rem 60px', textAlign: 'center', background: 'radial-gradient(ellipse at top, rgba(179,0,0,0.08) 0%, transparent 60%)' }}>
                <div style={{ display: 'inline-flex', alignItems: 'center', gap: 8, background: 'rgba(230,0,0,0.1)', border: '1px solid rgba(230,0,0,0.2)', padding: '4px 16px', borderRadius: 20, marginBottom: 20 }}>
                    <span style={{ color: '#e60000', fontWeight: 800, fontSize: '0.8rem', textTransform: 'uppercase', letterSpacing: 2 }}>📝 Blog</span>
                </div>
                <h1 style={{ fontSize: 'clamp(2rem, 6vw, 3.5rem)', fontWeight: 900, marginBottom: 16, lineHeight: 1.1 }}>
                    Conseils & Guide{' '}
                    <span style={{ color: '#e60000' }}>Location Voiture</span>
                </h1>
                    ...
                    <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(320px, 1fr))', gap: 28 }}>
                        {published.map(post => (
                            <a key={post.id} href={`/blog/${post.slug}`} style={{ textDecoration: 'none', display: 'block', background: '#111', borderRadius: 18, overflow: 'hidden', border: '1px solid rgba(255,255,255,0.06)', transition: 'all 0.3s ease', cursor: 'pointer' }}
                                onMouseEnter={e => { const el = e.currentTarget as HTMLElement; el.style.transform = 'translateY(-6px)'; el.style.boxShadow = '0 20px 50px rgba(0,0,0,0.4)'; el.style.borderColor = 'rgba(230,0,0,0.3)'; }}
                                ...
                                <div style={{ padding: '1.25rem' }}>
                                    ...
                                    {post.tags?.slice(0, 2).map(tag => (
                                        <span key={tag} style={{ display: 'inline-block', background: 'rgba(230,0,0,0.1)', color: '#ff6b6b', fontSize: '0.65rem', fontWeight: 700, padding: '2px 10px', borderRadius: 20, marginBottom: 10, marginRight: 6, textTransform: 'uppercase', letterSpacing: 0.5 }}>{tag}</span>
                                    ))}
                                    ...
                                    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                                        ...
                                        <span style={{ color: '#e60000', fontWeight: 800, fontSize: '0.8rem' }}>Lire l'article →</span>
                                    </div>
                                </div>
                            </a>
                        ))}
...
export default function Blog() {
    ...
    if (loading) return (
        <div style={{ background: '#0a0a0a', height: '100vh', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
            <div style={{ color: '#e60000', fontSize: '1.5rem', animation: 'pulse 1.5s infinite' }}>●</div>
        </div>
    );
    ...
    if (slug && slug !== '') {
        ...
        if (!post) return (
            <div style={{ background: '#0a0a0a', height: '100vh', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', color: '#fff', gap: 20 }}>
                ...
                <a href="/blog" style={{ color: '#e60000', fontWeight: 800, textDecoration: 'none' }}>← Retour au blog</a>
            </div>
        );
