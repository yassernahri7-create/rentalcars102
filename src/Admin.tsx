    ...
    return (
        <div dir="rtl" style={{
            ...
            <style>{`
                ...
                .login-input:focus { border-color: #e60000 !important; outline: none; }
            `}</style>
            ...
            <div style={{
                ...
                <div style={{ textAlign: 'center', marginBottom: '2.5rem' }}>
                        ...
                        <svg viewBox="0 0 100 100" style={{ position: 'absolute', inset: 0, width: '100%', height: '100%', animation: 'spinRing 10s linear infinite' }}>
                            ...
                            <defs>
                                <linearGradient id="loginRing" x1="0%" y1="0%" x2="100%" y2="100%">
                                    ...
                                    <stop offset="100%" stopColor="#e60000" stopOpacity="0.8" />
                                </linearGradient>
                            </defs>
                        </svg>
                    ...
                    <div style={{ fontWeight: 900, fontSize: '1.6rem', color: '#e60000', letterSpacing: '1px', lineHeight: 1.2 }}>NourCoches</div>
                ...
                <div style={{
                    background: '#111', border: '1px solid rgba(230,0,0,0.2)',
                    borderRadius: 24, padding: '2.5rem 2rem',
                    boxShadow: '0 24px 60px rgba(0,0,0,0.8), 0 0 0 1px rgba(230,0,0,0.05)',
                }}>
                    ...
                    <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: '1.2rem' }}>
                        ...
                        <button type="submit" disabled={submitting} style={{ padding: '1rem', borderRadius: 12, border: 'none', background: 'linear-gradient(45deg, #b30000, #e60000)', color: '#fff', fontWeight: 900, cursor: 'pointer', opacity: submitting ? 0.7 : 1 }}>
...
export default function Admin() {
    ...
    if (authed === null || (authed && loading)) {
        return <div style={{ background: '#0f0505', height: '100vh', display: 'flex', alignItems: 'center', justifyContent: 'center', color: '#e60000' }}>Loading...</div>;
    }
    ...
    return (
        <div dir="rtl" style={{ background: '#080000', color: '#fff', fontFamily: "'Cairo', sans-serif", minHeight: '100vh', paddingBottom: '5rem' }}>
            <style>{`
                ...
                .admin-header-outer { position: sticky; top: 0; z-index: 100; background: rgba(10,10,10,0.8); backdrop-filter: blur(15px); border-bottom: 1px solid rgba(230,0,0,0.2); padding: 1rem 2rem; }
            ...
            <div className="admin-header-outer">
                <div className="admin-header">
                    <div className="admin-header-logo">
                        ...
                        <div>
                            <h1 style={{ fontSize: '1.2rem', fontWeight: 900, color: '#e60000', margin: 0 }}>Admin Dashboard</h1>
                        </div>
                    </div>
                    <div className="admin-header-actions">
                        <div onClick={() => document.getElementById('promo-section')?.scrollIntoView({ behavior: 'smooth' })} style={{ display: 'flex', alignItems: 'center', gap: '6px', background: 'rgba(230,0,0,0.1)', padding: '6px 14px', borderRadius: 100, cursor: 'pointer', border: '1px solid rgba(230,0,0,0.2)' }}>
                            <span style={{ fontSize: '1.1rem' }}>Promo</span><span style={{ fontWeight: 900, color: '#ff4d4d' }}>{data.promoCars.length}</span>
                        </div>
                        ...
                        <div onClick={() => document.getElementById('blog-section')?.scrollIntoView({ behavior: 'smooth' })} style={{ display: 'flex', alignItems: 'center', gap: '6px', background: 'rgba(230,0,0,0.1)', padding: '6px 14px', borderRadius: 100, cursor: 'pointer', border: '1px solid rgba(230,0,0,0.2)' }}>
                            <span style={{ fontSize: '1.1rem' }}>📝</span><span style={{ fontWeight: 900, color: '#e60000' }}>{(data.posts || []).length}</span>
                        </div>
                        <a href="/" style={{ color: '#aaa', textDecoration: 'none', fontSize: '0.85rem', fontWeight: 700 }}>Website</a>
                        <button onClick={handleSave} disabled={saving} style={{ background: 'linear-gradient(45deg, #b30000, #e60000)', color: '#fff', border: 'none', padding: '0.7rem 1.5rem', borderRadius: 10, fontWeight: 900, cursor: 'pointer', opacity: saving ? 0.7 : 1 }}>{saving ? 'Saving...' : 'Save'}</button>
            ...
            <div className="admin-body">
                <div className="admin-stats-grid">
                    {[
                        ['Fleet', data.fleet.length, '#e60000'],
        ...
        <section className="admin-section" style={{ background: '#111', padding: '1.5rem', borderRadius: 20, marginBottom: '2rem' }}>
            ...
            <div style={{ display: 'grid', gap: '0.8rem' }}>
                {cars.map((car: any, i: number) => {
                    ...
                    return (
                        <div key={car.id} style={{ background: '#0a0a0a', borderRadius: 16, border: `1px solid ${isOpen ? '#e60000' : '#222'}`, overflow: 'hidden', transition: 'all 0.3s' }}>
                            {/* ── COLLAPSED HEADER ── */}
                            <div onClick={() => setExpanded(isOpen ? null : i)} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '0.8rem 1rem', cursor: 'pointer', background: isOpen ? 'rgba(230,0,0,0.05)' : 'transparent' }}>
                                ...
                                <div style={{ display: 'flex', gap: '0.4rem', alignItems: 'center', flexShrink: 0 }}>
                                    ...
                                    <button onClick={(e) => { e.stopPropagation(); if (confirm('حذف هذه السيارة؟')) { const n = cars.filter((_: any, idx: number) => idx !== i); if (onDelete) onDelete(n); else onChange(n); } }} style={{ background: '#ff000022', color: '#ff4444', border: 'none', borderRadius: 6, padding: '0.25rem 0.4rem', cursor: 'pointer', fontSize: '0.8rem' }}>🗑️</button>
                            ...
                            {isOpen && (
                                <div style={{ padding: '1rem', borderTop: '1px solid #222', animation: 'fadeIn 0.3s ease' }}>
                                    {uploading && <div style={{ textAlign: 'center', padding: '0.5rem', color: '#e60000', fontSize: '0.8rem', fontWeight: 800 }}>⏳ جارٍ رفع الصورة...</div>}
                                        ...
                                        <div style={{ display: 'flex', gap: '0.5rem', flexWrap: 'wrap', marginBottom: 6 }}>
                                            {(car.images || []).map((img: string, idx: number) => (
                                                <div key={idx} style={{ position: 'relative' }}>
                                                    ...
                                                    <button onClick={() => update(i, 'images', car.images.filter((_: any, j: number) => j !== idx))} style={{ position: 'absolute', top: -4, right: -4, background: '#e60000', color: '#fff', border: 'none', borderRadius: '50%', width: 18, height: 18, fontSize: '0.55rem', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>✕</button>
                                                </div>
                                            ))}
                                        </div>
                                    ...
                                    <div style={{ marginBottom: '1rem' }}>
                                        ...
                                        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(120px, 1fr))', gap: '0.5rem' }}>
                                            <div><label style={{ ...labelS, color: '#e60000' }}>AR</label><input type="text" value={typeof car.name === 'object' ? car.name.ar || '' : car.name} onChange={(e: any) => update(i, 'name', typeof car.name === 'object' ? { ...car.name, ar: e.target.value } : { ar: e.target.value, fr: '', en: '' })} style={iS} /></div>
                                    ...
                                    {/* ─ SLUG / DEEP LINK ─ */}
                                    <div style={{ marginBottom: '1rem', background: 'rgba(230,0,0,0.05)', padding: '0.8rem', borderRadius: 10, border: '1px dashed rgba(230,0,0,0.2)' }}>
                                        ...
                                        <div style={{ display: 'flex', gap: '0.5rem', alignItems: 'center' }}>
                                            ...
                                            <button
                                                ...
                                                style={{ background: '#e60000', color: '#fff', border: 'none', padding: '0.4rem 0.8rem', borderRadius: 6, fontSize: '0.75rem', fontWeight: 800, cursor: 'pointer' }}
                                            >حفظ الرابط 📋</button>
                                        </div>
                                    ...
                                    <div style={{ marginBottom: '1rem' }}>
                                        ...
                                        {(car.specs || []).map((spec: any, si: number) => (
                                            <div key={si} style={{ display: 'flex', gap: '0.4rem', marginBottom: 4, alignItems: 'center' }}>
                                                ...
                                                <button onClick={() => update(i, 'specs', car.specs.filter((_: any, j: number) => j !== si))} style={{ background: '#ff000033', color: '#ff4444', border: 'none', borderRadius: 6, padding: '0.3rem', cursor: 'pointer', fontSize: '0.7rem' }}>✕</button>
                                            </div>
                                        ))}
                    ...
                    return (
                        <div key={post.id} style={{ background: '#0a0a0a', borderRadius: 16, border: `1px solid ${isOpen ? '#fbbf24' : '#222'}`, overflow: 'hidden', transition: 'all 0.3s' }}>
                            ...
                            <div onClick={() => setExpanded(isOpen ? null : i)} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '0.8rem 1rem', cursor: 'pointer', background: isOpen ? 'rgba(251,191,36,0.05)' : 'transparent' }}>
                                ...
                                <div style={{ display: 'flex', gap: '0.4rem', alignItems: 'center', flexShrink: 0 }}>
                                    <button onClick={(e) => { e.stopPropagation(); if (confirm('Delete this post?')) { const n = posts.filter((_: any, idx: number) => idx !== i); if (onDelete) onDelete(n); else onChange(n); } }} style={{ background: '#ff000022', color: '#ff4444', border: 'none', borderRadius: 6, padding: '0.25rem 0.4rem', cursor: 'pointer', fontSize: '0.8rem' }}>🗑️</button>
                                    <span style={{ color: '#555', fontSize: '0.8rem', marginLeft: 4, transition: 'transform 0.3s', transform: isOpen ? 'rotate(180deg)' : 'rotate(0)' }}>▼</span>
                                </div>
                            </div>
